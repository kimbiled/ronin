const Proposal = () => {
   

    return (
        <div className="flex flex-col w-[90%] mx-auto rounded-xl gap-8">
        {/* Заголовок */}
            <div className="flex flex-col text-center mt-12 items-center">
                <p className="sm25:text-[40px] sm75:text-[40px] sm20:text-[36px] font-medium"><span className="text-[#1261FC]">Strong team</span> of<br />
                A-tier specialists</p>
                <p className="text-sm text-gray-500 sm20:w-[300px] sm20:text-xs sm25:text-sm sm75:text-sm sm75:w-[340px] sm25:w-[400px]">
                Hire a team of pros, pay the cost of one employee
                </p>
            </div>
            <div className="flex flex-col rounded-3xl h-auto"
        style={{
            background: "radial-gradient(271.63% 217.5% at 169.59% -28.68%, #22AFFF 0%, #1261FC 100%)"
          }}
        >
            <div className="flex flex-col items-center">
                <div className="relative bg-white rounded-xl sm20:w-[260px] sm75:w-[315px] sm25:w-[365px] h-36 mt-10">
                    <div>
                        <img src="./images/icons/star.png" alt="star"  className="w-12 h-12 absolute top-[-22px] left-5"/>
                    </div>
                    <div>
                        <img src="./images/icons/question.png" alt="question" className="w-6 h-6 absolute top-3 right-4 hover:opacity-80"/>
                    </div>
                    <div className="mt-4 flex flex-col p-4">
                       <div className="flex flex-row gap-3 items-center">
                            <p className="text-[34px]">2/5</p>
                            <p className="text-sm">client slots available</p>
                       </div>
                       <div className="w-42">
                            <p className="text-xs">We don't chase after quantity,<br />
                            but rather great results</p>
                       </div>
                    </div>
                </div>
                {/* splicer */}
                <div className="flex flex-row w-52 justify-between">
                    <div className="h-1 w-16 bg-white"></div>
                    <div className="h-1 w-16 bg-white"></div>
                </div>
                <div className="relative bg-white rounded-xl sm20:w-[260px] sm75:w-[315px] sm25:w-[365px] h-96">
                    <div className="mt-4 flex flex-col p-4">
                       <div className="flex flex-row gap-3 items-center sm20:w-[250px] sm75:w-[250px] sm25:w-[250px] ">
                            <p className="text-[34px] font-medium">Each plan contains</p>
                       </div>
                       <div className="flex flex-col gap-3 mt-3">
                            <div className="flex flex-row gap-2 rounded-2xl sm20:w-[230px] sm75:w-[260px] sm25:w-[275px] p-1 h-8 bg-[#F7F7F6] items-center">
                                <img src="./images/icons/tick.png" alt="tick"  className="h-6 w-6"/>
                                <p className="sm75:text-sm sm25:text-sm sm20:text-xs">2-3 business days delivery time</p>
                            </div>
                            <div className="flex flex-row gap-2 rounded-2xl sm20:w-[148px] sm75:w-[168px] sm25:w-[168px] p-1 h-8 bg-[#F7F7F6] items-center">
                                <img src="./images/icons/tick.png" alt="tick"  className="h-6 w-6"/>
                                <p className="sm75:text-sm sm25:text-sm sm20:text-xs">Whole team input</p>
                            </div>
                            <div className="flex flex-row gap-2 rounded-2xl sm20:w-[148px] sm75:w-[168px] sm25:w-[168px] p-1 h-8 bg-[#F7F7F6] items-center">
                                <img src="./images/icons/tick.png" alt="tick"  className="h-6 w-6"/>
                                <p className="sm75:text-sm sm25:text-sm sm20:text-xs">Unlimited brands</p>
                            </div>
                            <div className="flex flex-row gap-2 rounded-2xl sm20:w-[148px] sm75:w-[175px] sm25:w-[175px] p-1 h-8 bg-[#F7F7F6] items-center">
                                <img src="./images/icons/tick.png" alt="tick"  className="h-6 w-6"/>
                                <p className="sm75:text-sm sm25:text-sm sm20:text-xs">Unlimited revisions</p>
                            </div>
                            <div className="flex flex-row gap-2 rounded-2xl sm20:w-[180px] sm75:w-[200px] sm25:w-[200px] p-1 h-8 bg-[#F7F7F6] items-center">
                                <img src="./images/icons/tick.png" alt="tick"  className="h-6 w-6"/>
                                <p className="sm75:text-sm sm25:text-sm sm20:text-xs">Satisfaction guarantee</p>
                            </div>

                       </div>
                    </div>
                </div>
            </div>

            <div className="flex flex-col mt-6 items-center text-white p-8">
                <div className="flex flex-col gap-3 w-full ">
                    <div className=" flex flex-row justify-between items-center">
                        <h5 className="font-medium text-[25px]">No hiring fee</h5>
                        <img src="./images/icons/magic.png" alt="magic" className="h-6 w-6 brightness-125" />
                    </div>
                    <div className="border-b-[1px] pb-6 border-b-[#FFFFFF2E]">
                        <p className="opacity-70">We’re ready to get started right now,<br/>
                        as long as we have open spots</p>
                    </div>
                </div>
                <div className="flex flex-col gap-3 w-full mt-6">
                    <div className=" flex flex-row justify-between items-center">
                        <h5 className="font-medium text-[25px]">No interviews</h5>
                        <img src="./images/icons/magic.png" alt="magic" className="h-6 w-6 brightness-125"/>
                    </div>
                    <div className="border-b-[1px] pb-6 border-b-[#FFFFFF2E]">
                        <p className="opacity-70">Forget the endless search for<br/>
                        the best. We're already here</p>
                    </div>
                </div>
                <div className="flex flex-col gap-3 w-full mt-6">
                    <div className=" flex flex-row justify-between items-center">
                        <h5 className="font-medium text-[25px]">No extra hassle</h5>
                        <img src="./images/icons/magic.png" alt="magic" className="h-6 w-6 brightness-125"/>
                    </div>
                    <div>
                        <p className="opacity-70">Get near in-house experience<br/>
                        without having to think of benefits when hiring employees</p>
                    </div>
                </div>    
            </div>

            <div className="bg-white hover:bg-gray-100 rounded-xl sm20:w-[260px] sm75:w-[315px] sm25:w-[365px] h-12 mx-auto mb-12 flex flex-col items-center justify-center">
  <div className="flex flex-row items-center gap-2">
    <p className="font-medium">Get Proposal</p>
    <img
      src="./images/icons/arrow-right.png"
      alt="arrow-right"
      className="w-4 h-3 mt-1"
    />
  </div>
</div>
            </div>
      </div>
    );
};
  
export default Proposal;
